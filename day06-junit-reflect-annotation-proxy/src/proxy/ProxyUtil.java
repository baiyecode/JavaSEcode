 ...
 * ClassName: ProxyUtil
 * Package: proxy
 * Description: 代理工具类：中介公司，专门负责创建代理对象并返回给别人使用
 *
 * @Author 白夜
 * @Create 2025/7/26 15:11
 * @Version 1.0
 */
public class ProxyUtil {
    // 创建一个明星对象的代理对象返回。

    //Java内置的Proxy类只能为接口创建代理对象，不能直接为具体类创建代理对象
    public static StarService createProxy(Star s){
        /**
         * 参数一：用于执行用哪个类加载器去加载生成的代理类。
         * ProxyUtil.class.getClassLoader() 固定代码，获取当前类加载器
         * 参数二：用于指定代理类需要实现的接口: 明星类实现了哪些接口，代理类就实现哪些接口
         * s.getClass().getInterfaces() 固定代码,获取明星类实现的所有接口
